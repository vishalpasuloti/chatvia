...
export function Raghav() {
    ...
    async function saveData(data) {
        ...
        formData.append("userRaghavid", user.id);
    ...
    if (message) {
      ...
      if(docs){
        ...
        await axios.post("http://localhost:4000/addraghav", formData);
        reset();
        setMessage(""); 
        getRagchat(); 
    }
    ...
    async function getRagchat() {
        const url = "http://localhost:4000/getraghavchat/"+id;
        let response = await axios.get(url);
        setRagchat(response.data);
    }
...
<div className="row p-1 avi">
  ...
  <img src={require("../../assets/images/prof.jpg")} height="35" alt="" className="profile-user rounded-circle" /><span className="ms-2 mt-2" style={{ fontSize: "15px",fontWeight:'bold' }}>General</span>
  </div>
  <div className="col-lg-4 col-sm-8 p-3">
    <ul style={{display:"flex",flexDirection:'row',listStyle:'none'}}>
     ...
     <li>
     <button type="button" className="btn nav-btn" data-bs-toggle="modal" data-bs-target="#exampleModal1">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div className="" style={{textAlign:'center'}}>
        ...
        <div className=""><h3>Doris Brown</h3></div>
...
<div className=" mt-3" style={{ width: "871px",height: "430px"}}>
                        ...
                        {ragchat.map((item, index) => (
                            <div key={id} style={{marginBottom:"10px", textAlign: item.userRaghavid === user.id ? 'right' : 'left' }}>
{item.Message ? <span style={{padding:"5px",backgroundColor:"#7269ef",border:"1px solid none",color:"white"}}>{item.Message}</span> : null}
